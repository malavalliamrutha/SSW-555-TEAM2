			...
			if indi_count < 5000 and fam_count < 1000:
				...
				if arguments == "INDI":
					...
					if is_deathdate_lessthan_150years(current_indi["BIRT"]) == False:
						print(f"ERROR: Individual {current_indi['ID']} has a birth date {current_indi['BIRT']} more than 150 years old.")
					if is_birthdates_of_siblings(current_indi["BIRT"], current_indi["BIRT"]) == False:
						print(f"ERROR: Individual {current_indi['ID']} has a birth date {current_indi['BIRT']} less than 8 months to their sibling")
					if is_morethan_5_siblings_at_sametime(current_indi["BIRT"]) == False:
						print(f"ERROR: Individual {current_indi['ID']} has a birth date {current_indi['BIRT']} same as other 5 siblings")
					...
					if is_deathdate_lessthan_150years(current_indi["DEAT"]) == False:
						print(f"ERROR: Individual {current_indi['ID']} has a death date {current_indi['DEAT']} more than 150 years old after birth.")
					if is_birth_before_death(current_indi["BIRT"], current_indi["DEAT"]) == False:
						print(f"ERROR: Individual {current_indi['ID']} Birth {current_indi['BIRT']} should occur before death {current_indi['DEAT']}")
					...
					if parents_not_too_old(current_indi["BIRT"], current_indi["BIRT"], current_indi["BIRT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: Father {current_fam['HUSB']} is more than 80 years old than child")
				elif tag == "WIFE" and current_fam is not None:
					current_fam["WIFE"] = arguments.strip()
					if parents_not_too_old(current_indi["BIRT"], current_indi["BIRT"], current_indi["BIRT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: Mother {current_fam['WIFE']} is more than 60 years old than child")
				elif tag == "CHIL" and current_fam is not None:
					current_fam["CHIL"].append(arguments.strip())
					if is_birthdate_before_marrdate_ofparents(current_fam["MARR"], current_fam["DIV"], current_indi["BIRT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: {current_fam['CHIL']} was born: {current_indi['BIRT']} before marriage date: {current_fam['MARR']}")
					if birth_before_parents_death(current_indi["BIRT"], current_indi["DEAT"], current_indi["DEAT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: {current_fam['CHIL']} was born: {current_indi['BIRT']} after 9 months of the father's death")
					...
					if is_marriage_before_death(current_indi["DEAT"], current_fam["MARR"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: Married {current_fam['MARR']} after death {current_indi['DEAT']} of spouse.")
					if no_bigamy(current_fam["MARR"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: Marriage {current_fam['MARR']} should not occur during marriage to another spouse.")
					if is_marriage_before_divorce(current_fam["MARR"], current_fam["DIV"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: has a marriage date: {current_fam['MARR']} after the divorce date: {current_fam['DIV']}")
					if marriage_after_14(current_fam["MARR"], current_indi["BIRT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: Marriage {current_fam['MARR']} should be after 14 years of birth: {current_indi['BIRT']}")
					...
					if is_birthdate_before_marrdate_ofparents(current_fam["MARR"], current_fam["DIV"], current_indi["BIRT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: has a child after 9 months of divorce date: {current_fam['DIV']}")
					if is_divorce_before_death(current_fam["DIV"], current_indi["DEAT"]) == False:
						print(f"ERROR: Families {current_fam['ID']}: Divorce: {current_fam['DIV']} after spouse's death: {current_indi['DEAT']}")
