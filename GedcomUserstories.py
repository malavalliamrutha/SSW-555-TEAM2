...
def is_unique_first_names_in_families(individuals, families):
	...
	for individual_id, individual in individuals.items():
		if not isinstance(individual, dict):
			raise TypeError('Individual must be a dictionary')
		first_name = individual['name'].split()[0]
		if first_name in first_names:
			first_names[first_name].append(individual_id)
		else:
			first_names[first_name] = [individual_id]
	for family_id, family in families.items():
		if not isinstance(family, dict):
			raise TypeError('Family must be a dictionary')
		children_ids = family['children_ids']
		if children_ids:
			husband_first_name = individuals[family['husband_id']]['name'].split()[0]
			wife_first_name = individuals[family['wife_id']]['name'].split()[0]
			if husband_first_name in first_names and wife_first_name in first_names:
				children_first_names = []
				for child_id in children_ids:
					child_first_name = individuals[child_id]['name'].split()[0]
					children_first_names.append(child_first_name)
				if len(set(children_first_names)) != len(children_first_names):
					return False
	return True
...
#US30
def list_living_married(people, expected_output):
	living_married = []
	for person in people:
		if person["is_married"] and person["is_alive"]:
			living_married.append(person)
	assert living_married == expected_output, f"Expected {expected_output}, but got {living_married}."
	return living_married

#US29
def __init__(self, individuals):
        self.individuals = [Individual(**ind) for ind in individuals]
        self.write_to_file = []
...
def list_deceased(individuals):
	...
	for individual in individuals:
		if individual['is_deceased']:
			deceased_list.append((individual['name'], individual['death_date']))
	print("List of all the Deceased Individuals:")
	for name, death_date in deceased_list:
